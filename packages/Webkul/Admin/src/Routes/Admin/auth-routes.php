...
use Webkul\Admin\Http\Controllers\User\SocialAuthController;
...
Route::withoutMiddleware(['user'])->group(function () {
    ...
    Route::controller(SocialAuthController::class)->prefix('auth')->group(function () {
        Route::get('{provider}', 'redirect')->name('admin.social.redirect');

        Route::get('{provider}/callback', 'callback')->name('admin.social.callback');
    });
});
